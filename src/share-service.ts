...
export interface ShareAssistantAction {
  type: string;
  description: string;
  timestamp: string;
  promptId?: string;
...
export function transformToShareData(htmlData: HtmlData, sessionData: SessionData): ShareData {
  const { prompts, fileDiffs, assistantActions, sessionInfo, techStack } = htmlData;
  ...
  const shareData: ShareData = {
    ...
    assistantActions: assistantActions?.map(action => ({
      type: action.type,
